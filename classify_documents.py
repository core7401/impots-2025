...
def get_output_dir(config: dict) -> Path:
    return Path(config.get("output_dir", config["base_dir"]))
...
def guess_year(text: str, fallback: str = "2025") -> str:
    ...
    if matches:
        # on prend la dernière année trouvée
        return matches[-1]
    return fallback
...
def score_category(text_norm: str, filename_norm: str, categories: Dict[str, List[str]]) -> Tuple[str, Dict[str, int]]:
    scores: Dict[str, int] = {}
...
def score_apartment(text_norm: str, filename_norm: str, apartments: Dict[str, Dict[str, List[str]]]) -> Tuple[str, Dict[str, int]]:
    scores: Dict[str, int] = {}
...
def is_apartment_decision_confident(scores: Dict[str, int], min_apartment_score: int, min_score_gap: int) -> bool:
    if not scores:
        return False
...
def ensure_directories(
    ...
    print(f"[INFO] Vérification dossiers")
    inbox_dir.mkdir(parents=True, exist_ok=True)
    review_dir.mkdir(parents=True, exist_ok=True)

    for apartment in apartments.keys():
        for category in categories.keys():
            (output_dir / apartment / category).mkdir(parents=True, exist_ok=True)
...
def move_to_destination(file_path: Path, apartment: str, category: str, output_dir: Path, year: str) -> Path:
    destination_dir = output_dir / apartment / category
    ...
    while destination.exists():
        ...
        destination = destination_dir / f"{stem}_{counter}{file_path.suffix.lower()}"
        counter += 1
...
def process_file(file_path: Path, extractor: DocumentTextExtractor, ollama: OllamaClient, config: dict) -> None:
    ...
    destination = move_to_destination(file_path, final_apartment, final_category, output_dir, year)
...
def main() -> None:
    ...
    ensure_directories(inbox_dir, review_dir, output_dir, categories, apartments)
